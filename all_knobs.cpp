...
all_knobs_c::all_knobs_c() {
	...
	KNOB_RUN_THREAD_NUM = new KnobTemplate< unsigned > ("run_thread_num", 1); 
	
	/*lab5 knobs */

	KNOB_PERCEP_HIST_LEN = new KnobTemplate< unsigned > ("perceptron_his_len", 20);
	KNOB_PERCEP_TABLE_ENTRY = new KnobTemplate< unsigned > ("perceptron_table_len", 512);
	KNOB_PERCEP_CTR_BITS = new KnobTemplate< unsigned > ("perceptron_ctr_bits", 8);
	KNOB_PERCEP_THRESH_OVD = new KnobTemplate< unsigned > ("perceptron_threshold", 0);
...
void all_knobs_c::registerKnobs(KnobsContainer *container) {
	...
	container->insertKnob( KNOB_PERCEP_HIST_LEN);
...
void all_knobs_c::display() {
	...
	KNOB_PERCEP_HIST_LEN->display(cout);cout << endl;
