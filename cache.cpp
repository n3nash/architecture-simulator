...
int cache_read(Cache *cache, ADDRINT addr)
{
  ...
  ADDRINT cache_line = (ADDRINT) (addr >> (LOG2(cache->block_size)) );    /*cache line address this will change to physical address */
  int line_num   = cache_line % cache->entry_size;   /* cache index */
  ADDRINT tag        = (ADDRINT) cache_line / cache->entry_size;   /* cache tag   now store the physical tag  */ 
...
void cache_insert(Cache *cache, ADDRINT addr) {
  ...
  ADDRINT cache_line = (ADDRINT) (addr >> (LOG2(cache->block_size)) );    /*cache line address  change to physical*/
  int line_num   = cache_line % cache->entry_size;   /* cache index */
  ADDRINT tag        = (ADDRINT) cache_line / cache->entry_size;   /* cache tag change to physical */
